...
class Engine:
    ...
    def minimax(self, alpha, beta, depth, ind, return_move=False,time_limit=None,start_time=None):
        ...
        if self.board.turn == chess.WHITE:
            if depth >= 3 and not self.board.is_check() and self.endgame==0:
                ...
                score = self.minimax(beta - 1, beta, depth - 2, new_ind, False, time_limit, start_time)
                self.board.pop()
                if score is None:
                    return None
                if score >= beta:
            ...
            for move in moves:
                if depth >= 3 and ct >= 3 and not self.board.is_capture(move) and move not in self.killer_moves[depth] and not self.board.gives_check(move) and not self.board.is_check() and not move.promotion and self.endgame == 0:
                    ...
                    #value = self.minimax(alpha, beta, depth - 2, new_ind, False, time_limit, start_time)
                    value = self.minimax(alpha, beta, int(math.sqrt((len(moves) - ct) / len(moves)) * depth - 1), new_ind, False, time_limit, start_time)
                    if value is None:
                        self.board.pop()
                        return None
                    if value > alpha:
                        value = self.minimax(alpha, beta, depth - 1, new_ind, False, time_limit, start_time)
                else:
                    new_ind = self.push(move, ind)
                    value = self.minimax(alpha, beta, depth - 1, new_ind, False, time_limit, start_time)
                self.board.pop()
            ...
            if depth >= 3 and not self.board.is_check() and self.endgame==0:
                self.board.push(chess.Move.null())
                new_ind = ind ^ self.z_table[780]
                score = self.minimax(alpha, alpha + 1, depth - 2, new_ind, False, time_limit, start_time)
                self.board.pop()
                if score is None:
                    return None
                if score <= alpha:
            ...
            for move in moves:
                if depth >= 3 and ct >= 3 and not self.board.is_capture(move) and move not in self.killer_moves[depth] and not self.board.gives_check(move) and not self.board.is_check() and not move.promotion and self.endgame==0:
                    new_ind = self.push(move, ind)
                    #value = self.minimax(alpha, beta, depth - 2, new_ind, False, time_limit, start_time)
                    value = self.minimax(alpha, beta, int(math.sqrt((len(moves) - ct) / len(moves)) * depth), new_ind, False, time_limit, start_time)
                    if value is None:
                        self.board.pop()
                        return None
                    if value < beta:
    ...
    def iterative_deepening(self, max_depth, ind, time_limit):
        ...
        for depth in range(1, max_depth + 1):
            self.nodes = 0
            output = self.minimax(-999999999, 999999999, depth, ind, True, time_limit, start_time)
            if output is None:
